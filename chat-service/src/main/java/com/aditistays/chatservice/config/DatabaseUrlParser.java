...
/**
 * Converts the Python-style DATABASE_URL (postgresql+asyncpg://user:pass@host:port/db?opts)
 * into a JDBC URL + credentials, so the README's existing env var format keeps
 * working unchanged with the Java service.
 */
public final class DatabaseUrlParser {
    ...
    public static Parsed parse(String databaseUrl) {
        String normalized = databaseUrl.replace("postgresql+asyncpg://", "postgresql://");
        try {
